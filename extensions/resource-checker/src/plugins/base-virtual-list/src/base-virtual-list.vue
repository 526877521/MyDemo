...
import { unref, reactive, ref, computed, watchEffect, onUpdated, watch } from "vue";
...
const getCurrentTop = (index: number) => {
    ...
    if (state.cacheData.hasOwnProperty(index)) {
        return state.cacheData[index].top;
    } else if (state.cacheData.hasOwnProperty(index - 1)) {
...
watchEffect(() => {
    clientData.value.forEach((_, index) => {
        const currentIndex: number = state.start + index;
        if (state.cacheData.hasOwnProperty(currentIndex)) return;
